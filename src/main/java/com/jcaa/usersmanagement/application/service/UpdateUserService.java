...
public final class UpdateUserService implements UpdateUserUseCase {
  ...
  public UserModel execute(final UpdateUserCommand command) {
    ...
    // ARREGLADO: Mensaje genérico sin datos personales (PII)
    ...
    // ARREGLADO: Se eliminó el boolean flag y se llama directamente al método explícito
  ...
  private void ensureEmailIsNotTakenByAnotherUser(final UserEmail newEmail, final UserId ownerId) {
    // ARREGLADO: Condición ultra-legible y eficiente (solo consulta la DB una vez)
    getUserByEmailPort.getByEmail(newEmail).ifPresent(userWithEmail -> {
      if (!userWithEmail.getId().equals(ownerId)) {
